...
const CardDescription = ({ desc }: { desc: string }) => {
  if (desc.includes("[ Pendulum Effect ]") && desc.includes("[ Monster Effect ]")) {
    const pendulumMatch = desc.match(/\[ Pendulum Effect \](.*?)\[ Monster Effect \]/s);
    ...
    return (
      <Typography variant="body2" component="div" noWrap={false}>
        ...
        {pendulumText.split(/\r?\n/).map((line, idx) => (
          <span key={"pendulum-" + idx}>{line}<br /></span>
        ))}
        <br />
        <strong>Monster Effect</strong>
        <br />
        {monsterText.split(/\r?\n/).map((line, idx) => (
          <span key={"monster-" + idx}>{line}<br /></span>
        ))}
      </Typography>
    );
  }
